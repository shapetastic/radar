...
public sealed class EfficacyReadOnlyGuardrailTests
{
    ...
    // The walker lives in the shared TypeGraphClosure (spec 184 extracted it so the lifecycle boundary
    // guard does not carry a second, drifting copy); these delegates keep every call site above unchanged.
    private static HashSet<Type> TransitiveClosure(IEnumerable<Type> roots) =>
        TypeGraphClosure.TransitiveClosure(roots);

    private static IEnumerable<Type> ReferencedTypes(Type type) =>
        TypeGraphClosure.ReferencedTypes(type);
