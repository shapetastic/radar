...
/// <para>
/// <b>NEITHER SPEC 191 NOR SPEC 194 weakened this guard.</b> Spec 191 briefly made the stage-2 news
/// JUDGMENT a scoring input through one seam in <c>Radar.Application.SignalExtraction</c> whose
/// request/response types were Domain/BCL only. Spec 194 §1.1 DELETED that seam — it read a company
/// judgment while extracting an article the judgment had never seen — so the extraction and pipeline type
/// graphs are now further from the news subsystem than before, not closer. The judgment record, its status,
/// its trajectory enum and the judgment store all stay on the far side, and the ban below is unchanged.
/// <see cref="Radar.Application.Tests.SignalExtraction.KeywordSignalExtractorNewsNeutralityTests"/> asserts
/// the same boundary from the extraction side, including that no seam type survives dormant.
/// </para>
/// </summary>
