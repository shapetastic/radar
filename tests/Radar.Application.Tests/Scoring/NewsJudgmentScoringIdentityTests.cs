...
public sealed class NewsJudgmentScoringIdentityTests
{
    ...
    private const string CurrentMaterializerVersion = "news-judgment-signal-v3"; // spec 214 §2
    ...
    public void ChangingAStrengthConstantOrTheMapping_ProducesADifferentScoringConfigVersion(string knob)
    {
        ...
        var perturbed = knob switch
        {
            ...
            // A future materializer identity — deliberately NOT a retired one (v1/v2, values the
            // classifier still ACCEPTS), and deliberately not the shipped current one (spec 214 §2 moved
            // that on to v3).
            "materializer" => Enabled(materializerVersion: "news-judgment-signal-v4"),
