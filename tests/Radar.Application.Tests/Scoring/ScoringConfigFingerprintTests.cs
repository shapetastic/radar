...
public sealed class ScoringConfigFingerprintTests
{
    ...
    // scripts/run-profiles/default.json (commit 58c55f5); the rule-set identity is radar-keyword-rules-v5
    // (spec 128 adds the EquipmentAuthorization group; spec 127 added PatentActivity; spec 103 added
    // HiringActivity) while the enabled collector CSV stays this same 6-collector set (hiringats, patents, and
    // fccauth are all opt-in OFF).
    // Ordinal sort places "sec-13dg" before "sec-edgar" (the char after "sec-" is '1' 0x31 < 'e' 0x65).
    private const string SourceDescriptor =
        "rules=radar-keyword-rules-v5;collectors=RssPressReleaseCollector,newssearch,sec-13dg,sec-edgar,sec-form4,usaspending;";
    ...
    public void Compute_DefaultConfig_MatchesPinnedFingerprint()
    {
        ...
        // AD-10 replacement for the hand-bumped constant). This value is the spec-128 re-stamp: the
        // RuleSetVersion bumped radar-keyword-rules-v4 → v5 for the new EquipmentAuthorization rule group (an
        // opt-in-OFF FCC EAS collector), which the signal-source descriptor folds into the fingerprint
        // automatically. Scoring math is byte-identical (no existing evidence contains the FCC phrase; fccauth is
        // not in default.json) — only the input hash changes. It supersedes the spec-127 stamp
        // (radar-scoring-fp-b4a040144f66) and matches default.json's recorded default.
        var fp = ScoringConfigFingerprint.Compute(
            "mvp-engine-v1", "radar-formula-v8", new ScoringWeights(), DefaultTierDescriptor(), SourceDescriptor,
            InsiderDescriptor, MediaCollapseDescriptor);

        Assert.Equal("radar-scoring-fp-1251d4e0373e", fp);
    }
    ...
    public void Compute_ChangedSignalSourceDescriptor_ChangesFingerprint()
    {
        ...
        var changed = ScoringConfigFingerprint.Compute(
            ...
            "rules=radar-keyword-rules-v5;collectors=RssPressReleaseCollector,newssearch,sec-edgar,usaspending;",
            InsiderDescriptor, MediaCollapseDescriptor);
    ...
    public void Compute_AiOnDefault_MatchesPinnedFingerprint()
    {
        ...
        // rule group (opt-in OFF), which folds into BOTH defaults automatically (radar-scoring-fp-63c096e531ec)
        // → spec 128, the RuleSetVersion v4→v5 bump for the new EquipmentAuthorization rule group (opt-in-OFF
        // FCC EAS collector), which likewise folds into BOTH defaults with scoring math byte-identical.
        var fp = ScoringConfigFingerprint.Compute(
            "mvp-engine-v1", "radar-formula-v8", new ScoringWeights(), DefaultTierDescriptor(), AiOnSourceDescriptor,
            InsiderDescriptor, MediaCollapseDescriptor);

        Assert.Equal("radar-scoring-fp-2be98e738684", fp);
    }
