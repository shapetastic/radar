...
$dirArgs = [ordered]@{
    ...
    "Radar:AcquisitionsDirectory"    = (Join-Path $outRoot "acquisitions")       # spec 217 append-only recognised pending-acquisition records ({companyId}/{scanVersion}/{accession}.json since spec 227; only the current AcquisitionAgreementScan.Version governs); drives CompanyStatus.PendingAcquisition, report rule 0 and the CorporateActionInWindow efficacy exclusion
    "Radar:AcquisitionScanCacheDirectory" = (Join-Path $outRoot "acquisitions-cache")   # spec 217 heal-forward acquisition-scan answer cache (stamped with AcquisitionAgreementScan.Version), keyed by accession; stops the ~177 non-acquisition item-1.01 filings being re-fetched every run
