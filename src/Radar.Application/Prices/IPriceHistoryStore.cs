...
/// validation/backtest spec. Best-effort (AD-8): a disk/read failure logs and degrades (write reports a
/// <see cref="DurableWriteOutcome.Failed"/> outcome, read returns null) rather than throwing.
/// </summary>
public interface IPriceHistoryStore
{
    ...
    /// date. Best-effort (AD-8): a disk failure logs and never throws — the returned
    /// <see cref="DurableWriteResult"/> (spec 201 §1) carries the attempted path plus whether the file landed.
    /// </summary>
    Task<DurableWriteResult> WriteAsync(PriceHistory history, CancellationToken ct);
