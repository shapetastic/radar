...
/// <para>
/// When the opt-in price-history seam is enabled, an optional <see cref="IPriceHistoryAcquirer"/> is injected
/// (null when disabled, mirroring the runner's optional directional-filing source) and invoked as a SEPARATE
/// step after seeding — OUTSIDE <see cref="IRadarPipeline"/>. Price is validation/reference-only and can never
/// enter the evidence→signal→score path through this seam (AD-14).
/// </para>
/// </summary>
public sealed class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            // Opt-in price-history acquisition: a SEPARATE step OUTSIDE IRadarPipeline (AD-14). Null when the
            // Radar:Prices seam is disabled (the default), so this is a no-op and the pipeline is unchanged.
