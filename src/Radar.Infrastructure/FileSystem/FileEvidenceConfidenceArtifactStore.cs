...
/// <summary>Root directory for the spec-225 EvidenceConfidence measurement artifacts.</summary>
public sealed class FileEvidenceConfidenceArtifactStoreOptions
{
    public required string RootDirectory { get; init; }
}

/// <summary>
/// On-disk store for the spec-225 EvidenceConfidence measurement: writes
/// <c>{RootDirectory}/evidence-confidence.{json,csv,md}</c> through the shared
...
/// </para>
/// </summary>
public sealed class FileEvidenceConfidenceArtifactStore : IEvidenceConfidenceArtifactStore
{
    /// <summary>The fixed artifact file stem.</summary>
    private const string FileStem = "evidence-confidence";

    private readonly FileEvidenceConfidenceArtifactStoreOptions _options;
    private readonly ILogger<FileEvidenceConfidenceArtifactStore> _logger;

    public FileEvidenceConfidenceArtifactStore(
        FileEvidenceConfidenceArtifactStoreOptions options,
        ILogger<FileEvidenceConfidenceArtifactStore> logger)
    {
    ...
    public async Task<EvidenceConfidenceArtifactPaths> WriteAsync(
        string json, string csv, string markdown, CancellationToken ct)
    {
        ...
        if (jsonWritten)
        {
            _logger.LogInformation("Wrote EvidenceConfidence measurement JSON to {Path}.", jsonPath);
        }
        ...
        if (csvWritten)
        {
            _logger.LogInformation("Wrote EvidenceConfidence measurement CSV to {Path}.", csvPath);
        }
        ...
        if (markdownWritten)
        {
            _logger.LogInformation("Wrote EvidenceConfidence measurement markdown to {Path}.", markdownPath);
        }

        // Spec 201 §1: each file's outcome rides beside its attempted path — never a path as proof.
        return new EvidenceConfidenceArtifactPaths(
