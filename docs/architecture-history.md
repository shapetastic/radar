...
- **Spec 215 — company-reported reference values: the filing read keeps what it reads, and the judge is
  ...
  - **Identity.** Prompt v5, schema v4 and `reference-projection-v1` enter the `news=` segment (⚠ SPEC 216 SUPERSEDED TWO OF THE THREE — **AFTER they had stamped a live run**: prompt v5 → `news-judgment-prompt-v6` and `reference-projection-v1` → `v2`; schema v4 stands. The 214+215 operator step WAS performed on 2026-09-08, and this composition — `comparison-basis-v1`, prompt v5, schema v4, `reference-projection-v1`, with the reported-metrics ledger DISABLED by `766c925` so ZERO references were projected — stamped `radar-scoring-fp-8590412af27c` across 102 companies, strategy `default`, earliest score `2026-09-08T21:46:22.311Z`. That ONE-RUN COHORT is real accrued history: an analyst grepping that stamp finds it, so do not read 216 as having superseded a value that never reached disk. Spec 216 required a SECOND operator step of its own (since satisfied: `run-20260909T234658242Z-5c6644f6` stamped the spec-219 value). 216 also appends `rm=<policy>` to the DIRECTIONAL-FILING `ai=` descriptor, which is NOT this segment): the six
    ...
    `data/scoring-configs/strategies/{name}.json`, verify the first run's stamp against the test) was owed a
    SECOND time, separately (since satisfied: `run-20260909T234658242Z-5c6644f6` stamped the spec-219
    value, so no 216/217/219 step remains outstanding): the 214+215 step was ALREADY PERFORMED on 2026-09-08 and its composition
...
- **Spec 216 — a reference value must be PRIOR, DURABLE and VERIFIED before it can support a judgment: the
  ...
  - **§5 — identity.** `rm=disabled|<policy>` is appended LAST to the DIRECTIONAL-FILING `ai=` descriptor,
    ...
    cohort in KNOWINGLY; naming it here keeps its snapshots reconcilable. Spec 216 required a SECOND operator
    step (since satisfied: `run-20260909T234658242Z-5c6644f6` stamped the spec-219 value). ⚠ The spec's own line 43 ("the operator step (spec 214 §5) is performed once after merge") was
...
- **Spec 217 — a pending acquisition is a CLOSED THESIS: recognised deterministically from the 8-K, said
  ...
  - **⚠ BOTH PIN FAMILIES MOVED ONCE — the first time in the 197→216 arc that the AI-OFF side moved, and
    ...
    replaced are quoted there as history. **The operator step WAS OWED and is a THIRD, SEPARATE one** in this
    arc, not shared with 214/215 (whose step was taken 2026-09-08) or with 216 (which required its own) —
    since satisfied: `run-20260909T234658242Z-5c6644f6` stamped the spec-219 value, so no 216/217/219 step
    remains outstanding. The step was: delete
...
- **Spec 220 (2026-09-10) — the judge's family budget is filled by COMPARISON BASIS, not by syndication
  ...
  five `NotQuantified` families, and on the full cohort 7 of 17 cited trajectory facts sat outside the top
  five by member count (AEHR's at rank 28 of 50). `NewsJudgmentInputBuilder` now resolves each family's
  representative, classifies it ONCE with the existing `StatementComparisonClassifier` (no phrase table
  reimplemented, no model call added; the same read is the supplied `ComparisonBasis` line), and orders
  by basis rank — `StatedComparison` = `Event`, then `LevelOnly` (a level establishes no direction by
  itself, spec 214), then `NotQuantified` — then the pre-220 `MemberCount` → `DistinctPublisherCount` →
  `FamilyId` order, byte-identical within a class. Both cohorts read that one ordering; no budget changed.
  - **Identity.** `ordering=family-ordering-v2` is appended to `NewsJudgmentContract.CohortKey` after
    `references=`, because the order decides which facts a bounded judge sees. It forks the stage-2 cohort
    (no pre-220 verdict is reused) and, through the spec-194 §2 `news=` segment, moved the SIX AI-ON pins
  ...
  - **Record `news-judgment-v9`** (the tag spec 220 set; spec 221 moved `NewsJudgmentRecord.CurrentSchemaVersion` on to v10): `FamiliesAvailableByBasis`
    ...
    record could hold.
  - **§2 — a blank `challengeStrength` no longer discards a grounded judgment.** Six of sixteen 2026-09-09
    validation failures were `challenge-strength-out-of-range: ''` — an OMITTED number that binned the
    findings and rationale with it (the spec-192 defect in a new field). An absent strength beside
    surviving findings is now accepted, persisted as null (never 0, never a midpoint) and counted as
    `ChallengeStrengthNotStated` (one definition: `NewsJudgmentRecord.IsChallengeStrengthNotStated`); an out-of-range number still fails unchanged, and no other validation
    rule moved. It is NOT separately hashed, and it is NOT scoring-inert: the judgment-signal materializer
    gates on `Status == Judged`, so a formerly-discarded directional response whose basis is `Supported` or
    `ReferenceSupported` now mints a judgment signal. It needs no identity of its own only because, in THIS
    slice, it shares its comparability boundary with the `ordering=` cohort fork, which moves the cohort
    key and the AI-ON pins together — so no pre- and post-relaxation judgments are pooled under one stamp.
    That is why it was not split into its own spec; a later change to this rule ALONE would need its own
    identity. (Spec 192 is a precedent only for relaxing a DISPLAY-side discard, not a scoring one.) The
    live artifact renders "challenge strength not stated" rather than silence.
  - **Counted.** One aggregated line per (judge × stage-1 cohort) pass, beside the unchanged spec-219
    coverage line: `FamiliesByBasisAvailable`, `FamiliesByBasisSupplied` and `FamiliesWithheldByBudget`,
    each per basis class and split breadth / full, the count of inputs whose basis accounting was not
    recorded, and `ChallengeStrengthNotStated` by cohort — the last over judgments this pass actually
    called for only (spec 188 §1).
  - **OWED.** A FIFTH, separate operator step — after the 214+215, 216, 217 and 219 ones (delete or re-record every configured
    `data/scoring-configs/strategies/{name}.json` before the first post-220 baseline — git-ignored, NEVER
    fabricated; a `StrategyIdentityGuard` halt is correct if it is missed), and the spec-220 §4 before/after
