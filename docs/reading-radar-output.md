...
5. **Watch** — Opportunity at or above **the Lead's Watch line**, **or** the corroboration floor: an
   ...
   The type names on the "Why" line are the report's presentation labels (`EarningsTrajectory`,
   `InsiderActivity` — spec 211, `weekly-report-action-v4`), while the counting and grouping still run on
